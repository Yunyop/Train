  ...
  <a-modal v-model:visible="visible" title="请核对以下信息"
    ...
    <div class="order-tickets">
      ...
      <a-row class="order-tickets-row" v-for="ticket in tickets" :key="ticket.passengerId">
        ...
        <a-col :span="3">
            <span v-for="item in PASSENGER_TYPE_ARRAY" :key="item.code" :value="item.code">
      ...
      <div v-if="chooseSeatType===0" style="color: red;">
        您购买的车票不支持选座
        <div>12306规则：只有全部是一等座或全都是二等座才支持选票</div>
      ...
      <div v-else style="text-align: center">
        ...
        <div style="color: #999999">提示：您可以选择{{tickets.length}}个座位
        </div>
      </div>
<!--      <br>-->
<!--      最终购票：{{tickets}}-->
...
<!--      最终选座：{{chooseSeatObj}}-->
  ...
  <!--      第二层验证码 后段-->
    ...
    <a-button type="primary" danger block @click="handleOk">请输入验证码后开始购票</a-button>
  </a-modal>
  <!-- 第一层验证码 纯前端 -->
...
export default defineComponent({
  ...
  setup() {
    ...
    // 购票列表，用于界面显示，并传递到后端借口，用来描述：那个乘客购买什么座位的票
    // {
    //   passengerId:123,
    //   passengerType:"1",
    //   passengerName:"张三",
    //   passengerIdcard:"123231321132",
    //   seatTypeCode:"1",
    //   seat:"C1"
    // }
    ...
    // 根据选择的座位类型，计算出相应的列，比如要选的是一等座，就筛选出ACDF，要选的是二等座，就筛选出ABCDF
    const SEAT_COL_ARRAY=computed(()=>{
      return window.SEAT_COL_ARRAY.filter(itme=>itme.type===chooseSeatType.value);
    });
    // 选择的座位
    // {
    //   A1:false,C1:true,D1:false,F1:false,
    //   A2:false,C2:true,D2:false,F2:false
    // }
    ...
    const finishCheckPassenger=()=>{
      ...
      // 检验余票是否充足，购票列表中的每个座位类型，都去车次座位余票信息中，看余票是否充足
      // 前端校验不一定准，但前端校验可以减轻后端很多压力
      //必须copy出seatTypesTemp变量来扣减，用原始的seatType去扣减会影响真实的库存
      ...
      for (let i=0;i<tickets.value.length;i++){
        ...
        for (let j = 0; j < seatTypesTemp.length; j++) {
          ...
          // 同类型余票减一，这里扣减出来的是临时变量，不是真正的库存，只是为了校验
      ...
      if (ticketSeatTypeCodesSet.length!==1){
        console.log("选了多种座位不支持选座");
        chooseSeatType.value=0;
      }
      else {
        // ticketSeatTypeCodesSet.length===1,既只选了一种座位（不是一个座位，是一种座位）
    ...
    //   设置每张票的座位
    //   先清空购票列表的座位，有可能之前选了并设置座位了，但座数不对被拦截了，又重新选一遍
      for (let i = 0; i < tickets.value.length; i++) {
        tickets.value[i].seat=null;
      }
      let i=-1;
      // 要么不选座，要么所选座位应该等于购票数，既i===(tickets.value.length-1)
      ...
      axios.post("/business/web/confirm-order/do", {
    ...
    const queryLineCount=()=>{
      ...
      queryLineCountInterval=setInterval(function () {
        axios.get("/business/web/confirm-order/query-line-count/"+confirmOrderId.value).then((response)=>{
          let data=response.data;
          if (data.success){
            ...
            switch (result) {
              ...
              case -2:
                ...
                break;
              case -3:
                notification.error({description:"抱歉没票了"});
                lineModalVisible.value=false;
                clearInterval(queryLineCountInterval);
                break;
              default:
...
.order-tickets .order-tickets-header{
  ...
  booder:solid 1px cor cornflowerblue;
  color: white;
  font-size: 16px;
  padding: 5px 0;
}
